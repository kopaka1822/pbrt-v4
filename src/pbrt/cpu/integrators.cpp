...
// Restir Method Definitions
RestirIntegrator::RestirIntegrator(int maxDepth, Camera camera, Sampler sampler,
                               Primitive aggregate, std::vector<Light> lights,
                               const std::string &lightSampleStrategy, bool regularize)
    : RayIntegrator(camera, sampler, aggregate, lights),
      maxDepth(maxDepth),
...
std::string RestirIntegrator::ToString() const {
    return StringPrintf("[ RestirIntegrator maxDepth: %d lightSampler: %s regularize: %s ]",
                        maxDepth, lightSampler, regularize);
}
...
std::unique_ptr<RestirIntegrator> RestirIntegrator::Create(
    ...
    std::string lightStrategy = parameters.GetOneString("lightsampler", "bvh");
    bool regularize = parameters.GetOneBool("regularize", false);
    return std::make_unique<RestirIntegrator>(maxDepth, camera, sampler, aggregate, lights,
                                            lightStrategy, regularize);
}
...
std::unique_ptr<Integrator> Integrator::Create(
    ...
    if (name == "path")
        ...
        integrator = RestirIntegrator::Create(parameters, camera, sampler, aggregate, lights, loc);
